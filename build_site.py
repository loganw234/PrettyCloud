...
Writes the site pages into the repo root. The atlas lives at
/atlas/ and is reached from the banner and from "Open the atlas".
"""
import html
from pathlib import Path

HERE = Path(__file__).resolve().parent  # the repo root now
...
<head>
<meta charset="utf-8">
<!-- OLD LINKS STILL WORK. The atlas used to be at the root, so a
     bookmark, a printed card or a shared frame arrives here carrying
     ?plate=... . Forward it, query and hash intact, before the page
     renders - the visitor asked for a frame, not for a front door. -->
<script>(function(){{var q=location.search;
  if(q&&/[?&]plate=/.test(q))location.replace("/atlas/"+q+location.hash);}})();</script>
...
MOVED_BANNER = """<div id="moved">
  <p><b>Looking for the atlas?</b> It has not gone anywhere. The toy now
  lives at <span class="mono">/atlas</span>, and every old link still
  works.</p>
  <a class="go" href="/atlas/">Open the atlas</a>
  <button type="button" id="movedX" aria-label="Dismiss this notice">&times;</button>
</div>
<script>(function(){var b=document.getElementById("moved");
  if(!b)return;
  /* SHOWN BY DEFAULT and hidden by script, never the other way round:
     built the other way, anybody with JS off would never see it. */
  try{if(localStorage.getItem("movedSeen"))b.remove();}catch(e){}
  var x=document.getElementById("movedX");
  if(x)x.onclick=function(){try{localStorage.setItem("movedSeen","1");}catch(e){}
    b.remove();};})();</script>
"""
...
def page(slug, title, desc, body):
    ...
    return SHELL.format(title=html.escape(title), desc=html.escape(desc),
                        url=f"{SITE}/{slug}", card=CARD, nav=nav,
                        banner=MOVED_BANNER if slug == "index.html" else "",
                        body=body)
...
PAGES["index.html"] = dict(
    ...
    desc="A film camera for things that have never existed, with real "
         "glass in front of it. Sixty-eight mathematical objects, "
...
<figure>
  ...
  <figcaption>Plate I, the Hopf fibration, photographed through the
  1840 Petzval portrait at f/3.67. Every closed curve here is the
  preimage of one point on an ordinary sphere, and any two of them,
  chosen anywhere, are linked exactly once. The atlas itself draws all
  of that perfectly sharp, because it has no lens: every point lands
  where the arithmetic says. Putting real glass in front spoils that on
  purpose. A Petzval cannot flatten its field, so the corners are
  focused at a different distance from the middle and go soft, and the
  out-of-focus highlights stretch as the pupil is seen at an angle.
  Neither is an effect applied afterwards. It is what those radii
  do.</figcaption>
</figure>

<div class="prose">
<p>The subjects are mathematical objects: sixty-eight of them, each a few
...
<div class="cols">
  <div>
    ...
    <p>The atlas runs in a browser. Sixty-eight objects, drawn a million
