...
Atlas.registerPlate({
  ...
  caption: "Take a row of cells, black or white, and let each become the exclusive-or of its left neighbour with the or of itself and its right. That is the whole law: rule thirty of the two hundred fifty-six, in the numbering Wolfram published in 1983, though it took him until 1984 to see what this one was doing. Grow it from one black cell and the left flank settles into tidy diagonals while the rest dissolves into churn that passes the standard batteries of randomness tests; Mathematica shipped the centre column as its random integers. Nobody can say why. In 2019 Wolfram put thirty thousand dollars on three questions about that column alone, and all three still stand. What is believed, and prized, is the price: the rule is computationally irreducible, and the only road to row ten thousand runs through the ten thousand rows before it. This plate pays full fare - every point of light re-lives the whole history from the seed down to the cell it lands on, the run wound on a register of up to five hundred twelve cells so the growing cone finally meets itself round the ring and the weave begins - and DEPTH prices the rows in powers of two. Ninth of the Mk2 series, and its conscience: the one plate whose depth cannot be addressed, only earned.",
